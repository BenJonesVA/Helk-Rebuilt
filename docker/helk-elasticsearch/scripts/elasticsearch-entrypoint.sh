...
# Check to make sure not set in docker config/runtime. Heap size is set purely via
# ES_JAVA_OPTS (not by editing config/jvm.options, which ships as the image's
# JDK-version-tuned default and is left untouched).
if [[ -z "$HELK_ES_MEMORY" ]]; then
  ...
  export ES_JAVA_OPTS="${ES_JAVA_OPTS} -Xms${HELK_ES_MEMORY} -Xmx${HELK_ES_MEMORY} "
  echo -e "${HELK_INFO_TAG} Setting ES_JAVA_OPTS to ${ES_JAVA_OPTS} from HELK_ES_MEMORY"
fi
